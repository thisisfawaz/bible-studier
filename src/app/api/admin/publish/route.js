...
const SECRET_ADMIN_PATH = 'kamiurban24'; // ← Hardcode your secret path here

export default function AdminDashboard() {
  const params = useParams();
  const router = useRouter();
  const adminPath = params.adminPath;
  const expectedPath = SECRET_ADMIN_PATH;
  ...
  const publishDevotion = async (devotion) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Force a full reload of devotions
        await loadDevotions();
        // Also force a page refresh of the list by updating state
        setDevotions(prev => ({
          ...prev,
          published: [...prev.published, data.devotion]
        }));
      } else {
        alert('❌ Failed: ' + (data.error || 'Unknown error'));
      }
